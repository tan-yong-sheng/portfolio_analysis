...
\end_inset
 ...
 estimates computed for the models produced using 2 different methods.
 When models contain entity effects and are not weighted, the two methods
 agree.
\end_layout

\begin_layout Standard
\noindent
The correlation-based measures 
\series bold
do not
\series default
 make use of weights.
 They are prefixed as 
\begin_inset Formula $\texttt{corr\_squared}$
\end_inset
...
\begin_layout Standard
Define 
\begin_inset Formula $\bar{y}_{iw}$
\end_inset

 and 
\begin_inset Formula $\bar{x}_{iw}$
\end_inset

 to be entity-wise weighted means, and the entity-wise weight to be
\end_layout
...
\noindent
where 
\begin_inset Formula $\ddot{\bar{y}}_{iw}=\bar{y}_{iw}$
\end_inset

 if the model does not contains a constant or 
\begin_inset Formula $\ddot{\bar{y}}_{iw}=\bar{y}_{iw}-\bar{\bar{y}}_{w}$
\end_inset

 if the model does.
 The overall weighted mean is defined 
\begin_inset Formula 
\[
...
\begin_layout Standard
\noindent
This measure matches Stata.
 It does not reflect weighting.
\begin_inset Formula 
\[
...
\begin_layout Standard
\noindent
if the model contains a constant or 
\begin_inset Formula $\bar{\bar{y}}_{w}=0$
\end_inset
...
\noindent
Define 
\begin_inset Formula $\tilde{y}_{it,w}=\sqrt{w_{it}}\left(y_{it}-\bar{y}_{iw}\right)$
\end_inset

 and 
\begin_inset Formula $\tilde{x}_{it,w}=\sqrt{w_{it}}\left(x_{it}-\bar{x}_{iw}\right)$
\end_inset
